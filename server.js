...
const { Dog, match_dogs } = require('./dogMatching');
...
app.post('/api/upload', (req, res) => {
  ...
  `
    ...
    .catch((err) => {
      ...
      if (err.message.includes('violates some condition')) {
        res.status(400).json({ error: 'Неверные данные для сохранения изображения' });
      } else {
